...
export default function ManageBeforeAfterModal({ programId, programName, docs, onClose, onSaved }: Props) {
  ...
  const slotBox = (slot: PickSlot) => {
    ...
    const d = docById(id)
    const t = thumb(d)
    const accent = slot === 'before' ? '#DC2626' : '#059669'
    return (
      <div style={{ flex: 1 }}>
              ...
              return (
                <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 8, borderRadius: 10, border: '1px solid var(--border-subtle)', background: 'var(--card)' }}>
                  ...
                  <button
                    ...
                    style={{ flexShrink: 0, width: 30, height: 30, borderRadius: 8, border: 'none', background: 'rgba(220,38,38,0.1)', color: '#DC2626', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'inherit' }}
                  >
